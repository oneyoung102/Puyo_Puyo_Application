...
puyoEndingPage::puyoEndingPage(puyoFileSystem& pfs, int win_player_num, sf::Sprite capture_sprite, Arcade arcade)
    : buttons(decltype(buttons)({{buttonName::replay,buttonName::to_menu}}))
    ...
    switch(proceed_count)
    {
        ...
        case BUTTON_APPEAR_TICK:
        {
            pp.add_print_button(make_unique<puyoPrintButton>(
                ...
                buttons.cursor.get_select_status(buttonName::replay),
                REPLAY_BUTTON_POS,
                "Replay",
                pfs.get_font(),
                REPLAY_BUTTON_SCALE));
            pp.add_print_button(make_unique<puyoPrintButton>(
                pfs.get_sprite(puyoFileSystem::Image::button),
                buttons.cursor.get_select_status(buttonName::to_menu),
                TO_MENU_BUTTON_POS,
                "To Menu",
                pfs.get_font(),
