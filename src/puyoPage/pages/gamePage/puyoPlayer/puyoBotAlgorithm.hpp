...
class puyoBotAlgorithm
{
    private :
        ...
        std::vector<std::pair<int,int>> calc_all_probablities(const puyoBoard& board); //{좌우이동, 화전수}
        std::tuple<POSi,POSi> to_coord(std::pair<int,int> probablity, const puyoPlayPuyo& puyo); //가능성을 좌표로 변환
