  ...
  for(int i = 0; i < board_c; ++i)
  {
    ...
    for(int j = board_r - 1; j >= 0; --j) // 아래에 있는 뿌요가 먼저 오게
    {
      ...
      if(puyo != Type::blank)
      {
        if(push)
        {
          push_gravity_puyo(std::move(puyoGravityPuyo(i, j, (int)puyo, BOARD_FALL_GRAVITY_TICK)));
          remove_puyo(j, i);
          continue;
        }
      } else
...
void puyoBoard::find_vanish_puyo()
{
  ...
  for(int i = 0; i < board_r; ++i)
    for(int j = 0; j < board_c; ++j) {
     ...
     int puyo_count_except_obsp =
          stored_puyos.size() - obstruct_puyo_counting;
      if(puyo_count_except_obsp >= condition_for_vanish) {
        ...
        for(const auto [r, c, type] : stored_puyos)
        {
          if(type == Type::obstruct)
            push_vanish_puyo(std::move(puyoVanishPuyo(c, r, (int)type, BOARD_OBSTRUCT_VANISH_TICK)));
          else
            push_vanish_puyo(std::move(puyoVanishPuyo(c, r, (int)type, BOARD_BASIC_VANISH_TICK)));
          remove_puyo(r, c);
        }
      }
    }
...
{
  obstruct_puyo = min(max(0, obstruct_puyo + count), obstruct_puyo_max);
}
void puyoBoard::spawn_obstruct_puyo(int obstruct_puyo_for_dropping)
{
  ...
  while (obstruct_puyo_for_dropping > 0)
  {
    ...
    --obstruct_puyo;
    gravity_puyos.push_back(puyoGravityPuyo(col, -obstruct_puyo_height[col] - obps_spawn_y,(int)Type::obstruct,BOARD_FALL_GRAVITY_TICK));
...
void puyoBoard::find_energy_puyos(float fx, float fy, float tx, float ty)
{
  ...
  for(const auto [x, y, color] : temp_energy_puyos)
    energy_puyos.push_back(puyoEnergyPuyo(fx + x * PUYO_SIZE, fy + y * PUYO_SIZE, tx, ty, color, BOARD_FLY_TICK));
